...
public final class TreeBuilder: TokenSink {
    ...
    private func processCharacter(_ ch: Character) {
        ...
        if self.isInMathMLTextIntegrationPoint() || self.isInSVGHtmlIntegrationPoint() || self.isInMathMLAnnotationXmlIntegrationPoint() {
            if ch == "\0" {
    ...
    private func processStartTag(name: String, attrs: [String: String], selfClosing: Bool) {
        ...
        case .inTableBody:
            if name == "tr" {
                ...
                if !self.hasElementInTableScope("tbody"), !self.hasElementInTableScope("thead"), !self.hasElementInTableScope("tfoot") {
                    self.emitError("unexpected-start-tag")
                    return
                }
                self.clearStackBackToTableBodyContext()
        ...
        case .inSelect:
            if name == "html" {
                ...
                let selectIsContextOnly = self.contextElement?.name == "select" &&
    ...
    private func processEndTag(name: String) {
        ...
        case .inSelect:
                ...
                name.lowercased() == "button" || name.lowercased() == "datalist"
            {
                ...
                self.emitError("unexpected-end-tag")
                if self.openElements.contains(where: { $0.name == name.lowercased() }) {
    ...
    private func processEndTagInBody(name: String) {
        if name == "body" {
            ...
            if !self.hasElementInScope("h1"), !self.hasElementInScope("h2"), !self.hasElementInScope("h3"),
            ...
            let hasTemplate = self.openElements.contains { $0.name == "template" && ($0.namespace == nil || $0.namespace == .html) }
            if !hasTemplate {
                self.emitError("unexpected-end-tag")
                return
            }
    ...
    private func insertCharacter(_ ch: Character) {
        ...
        if self.fosterParentingEnabled, ["table", "tbody", "tfoot", "thead", "tr"].contains(target.name) {
    ...
    private func popUntil(_ name: String) {
        ...
        let isContextOnly = self.contextElement?.name == name &&
            !self.openElements.contains { $0.name == name && ($0.namespace == nil || $0.namespace == .html) }
    ...
    private func reconstructActiveFormattingElements() {
        ...
        while entryIndex < self.activeFormattingElements.count {
            ...
            let newElement = self.insertElement(name: entry.name, namespace: entry.namespace ?? .html, attrs: entry.attrs)
    ...
    private func adoptionAgency(name: String) {
        ...
        for _ in 0 ..< 8 {
            ...
            } else if self.fosterParentingEnabled, ["table", "tbody", "tfoot", "thead", "tr"].contains(commonAncestor.name) {
    ...
    private func hasForeignElementInStack() -> Bool {
        return self.openElements.contains { elem in
