...
private struct SelectorParser {
	...
	mutating func parseCompoundSelector() throws -> Selector {
		...
		if simples.isEmpty {
			throw SelectorError("Expected selector", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
	...
	mutating func parseSimpleSelector() throws -> SimpleSelector? {
		switch self.current {
			...
			case "#":
				...
				if name.isEmpty {
					throw SelectorError("Expected id name after #", position: self.input.distance(from: self.input.startIndex, to: self.position))
				}
				return .id(name)
			...
			case ".":
				...
				let name = self.parseName()
				if name.isEmpty {
					throw SelectorError("Expected class name after .", position: self.input.distance(from: self.input.startIndex, to: self.position))
				}
				return .class(name)
	...
	mutating func parseAttributeSelector() throws -> AttributeSelector {
		guard self.consume("[") else {
			throw SelectorError("Expected [", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
		...
		if attrName.isEmpty {
			throw SelectorError("Expected attribute name", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
	...
	mutating func parseQuotedString() throws -> String {
		...
		guard self.consume(quote) else {
			throw SelectorError("Unterminated string", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
	...
	mutating func parsePseudoSelector() throws -> SimpleSelector {
		...
		switch name {
			...
			case "not":
				guard self.consume("(") else {
					throw SelectorError("Expected ( after :not", position: self.input.distance(from: self.input.startIndex, to: self.position))
				}
				...
				self.skipWhitespace()
				guard self.consume(")") else {
					throw SelectorError("Expected ) after :not selector", position: self.input.distance(from: self.input.startIndex, to: self.position))
				}

				return .pseudoClass(.not(inner))

			default:
				throw SelectorError("Unknown pseudo-class: \(name)", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
	}

	mutating func parseNth() throws -> (Int, Int) {
		guard self.consume("(") else {
			throw SelectorError("Expected ( for nth expression", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
		...
		self.skipWhitespace()
		guard self.consume(")") else {
			throw SelectorError("Expected ) after nth expression", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
	...
	mutating func parseAnPlusB() throws -> (Int, Int) {
		...
		if self.current == "n" || self.current == "N" {
			...
			if self.current == "n" || self.current == "N" {
				...
				self.skipWhitespace()
				guard self.consume(")") else {
					throw SelectorError("Expected ) after number", position: self.input.distance(from: self.input.startIndex, to: self.position))
				}
			...
			throw SelectorError("Invalid nth expression", position: self.input.distance(from: self.input.startIndex, to: self.position))
		}
