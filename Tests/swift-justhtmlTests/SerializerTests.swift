...
func serializeSerializerTokenStream(_ tokens: [[Any]], options: [String: Any] = [:]) -> String? {
	...
	let stripWhitespace =
		options["strip_whitespace"] as? Bool ?? (options["strip_whitespace"] as? Int == 1)
	let injectMetaCharset =
		options["inject_meta_charset"] as? Bool ?? (options["inject_meta_charset"] as? Int == 1)
	let encoding = options["encoding"] as? String
	let useTrailingSolidus =
		options["use_trailing_solidus"] as? Bool ?? (options["use_trailing_solidus"] as? Int == 1)
	...
	for i in 0 ..< tokens.count {
		...
		switch kind {
			...
			case "EndTag":
				...
				// Track preserve whitespace elements
				if ["pre", "textarea", "script", "style"].contains(name) {
					preserveWhitespace = openElements.contains(where: {
						["pre", "textarea", "script", "style"].contains($0)
					})
				}
			...
			case "EmptyTag":
				...
				if injectMetaCharset, let enc = encoding, name == "meta" {
					...
					// Check if this is an http-equiv content-type meta tag
						...
						if let content = attrs["content"] {
							...
							let charsetPattern = try? NSRegularExpression(
								pattern: "charset=[^;\\s]+", options: .caseInsensitive)
							if let regex = charsetPattern {
								let range = NSRange(content.startIndex..., in: content)
								let newContent = regex.stringByReplacingMatches(
									in: content, options: [], range: range, withTemplate: "charset=\(enc)")
				...
				parts.append(
					serializeStartTag(
						name, attrs: attrs, options: options, isVoid: true, useTrailingSolidus: useTrailingSolidus
					))
...
func serializeStartTag(
	_ name: String, attrs: [String: String], options: [String: Any], isVoid: Bool,
	useTrailingSolidus: Bool = false
) -> String {
	var result = "<\(name)"
...
func serializeAttribute(_ name: String, value: String, options: [String: Any]) -> String {
	let quoteAttrValues =
		options["quote_attr_values"] as? Bool ?? (options["quote_attr_values"] as? Int == 1)
	let escapeLtInAttrs =
		options["escape_lt_in_attrs"] as? Bool ?? (options["escape_lt_in_attrs"] as? Int == 1)
	let customQuoteChar = options["quote_char"] as? String
	let minimizeBooleanAttrs =
		options["minimize_boolean_attributes"] as? Bool
			?? (options["minimize_boolean_attributes"] as? Int != 0)
