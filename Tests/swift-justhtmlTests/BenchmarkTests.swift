...
@Test func benchmarkComprehensiveSummary() async throws {
	...
	for result in results {
		...
		print(
			"| \(namePadded) | \(sizePadded) | \(String(format: "%10d", result.iterations)) | \(String(format: "%8.3f", result.avgMs)) | \(String(format: "%8.3f", result.minMs)) | \(String(format: "%8.3f", result.maxMs)) | \(String(format: "%4.2f", result.throughputMBs)) |"
		)
	}
	...
	for result in results {
		#expect(
			result.avgMs < 500, "\(result.name) should complete in under 500ms (was \(result.avgMs)ms)")
	}
}
...
func generateEntityHTML(count: Int) -> String {
	...
	for i in 0 ..< count {
		html +=
			"<p>Entity test \(i): &amp; &lt; &gt; &quot; &nbsp; &#60; &#x3E; &copy; &reg; &trade;</p>"
	}
	html += "</body></html>"
	return html
}
